    ...
    return (
        <div style={{ padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '10px' }}>
                <Link to={`/add`}>
                    <button className='btn btn-primary' style={{ marginRight: '8px' }}>add task</button>
                </Link>
            </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #ddd' }}>
                <thead>
                    <tr style={{ backgroundColor: '#f5f5f5', borderBottom: '2px solid #ddd' }}>
                        <th style={{ padding: '12px', textAlign: 'center', fontWeight: 'bold' }}>Title</th>
                        <th style={{ padding: '12px', textAlign: 'center', fontWeight: 'bold' }}>Description</th>
                ...
                <tbody>
                    {tasks?.map((task) => (
                        <tr key={task._id} style={{ borderBottom: '1px solid #ddd' }}>
                            <td style={{ padding: '12px', textAlign: 'center' }}>{task.title}</td>
                            <td style={{ padding: '12px', textAlign: 'center' }}>{task.description}</td>
                            <td style={{ padding: '12px', textAlign: 'center' }}>{task.status}</td>
                            <td style={{ padding: '12px', textAlign: 'right' }}>
                                <Link to={`/details/${task._id}`}>
                                    <button className='btn btn-primary' style={{ marginRight: '8px' }}>View</button>
                                </Link>
